...
class IncomeStatementPage extends StatelessWidget {
  ...
  static Route route(RouteSettings settings) {
    ...
    return MaterialPageRoute(
      settings: settings,
      builder: (_) => IncomeStatementPage(organization: organization),
    );
  }
  ...
  Widget build(BuildContext context) {
    context
        .read<IncomeStatementBloc>()
        .add(IncomeStatementFetchEvent(organization: organization));

    return Scaffold(
      appBar: AppBar(
        centerTitle: true,
        title: const Text('Demonstração de Resultado'),
      ),
      body: Center(
            ...
            if (state is Loaded) {
              return SingleChildScrollView(
                child: DataTable(
                  ...
                  rows: state.incomeStatementRows
                      .map(
                        (e) => DataRow(
                          cells: [
                            DataCell(
